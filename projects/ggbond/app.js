...
void main(){
 ...
 col+=vec3(1.,.94,.78)*line*inside(su)*sub.a*sweep*uFoil*0.0;
...
function reset(){targetX=.025;targetY=-.13;targetZoom=1;flipped=false;setAuto(false);$('view-label').textContent='FRONT · 正面';$('flip').innerHTML='翻看背面 <span>↻</span>';resize();}
...
function setupControls(){
 for(const [id,name,label] of [['foil','uFoil','foil-value'],['scale','uScale','scale-value']]){const input=$(id);input.value=uniforms[name].value;const update=()=>{uniforms[name].value=Number(input.value);$(label).value=id==='foil'?Math.round(input.value*100)+'%':Number(input.value).toFixed(2);};input.addEventListener('input',update);update();}
 ...
 stage.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','f','F','r','R'].includes(e.key)){e.preventDefault();setAuto(false);}const base=flipped?Math.PI:0;if(e.key==='ArrowLeft')targetY-=.07;if(e.key==='ArrowRight')targetY+=.07;if(e.key==='ArrowUp')targetX-=.06;if(e.key==='ArrowDown')targetX+=.06;if(e.key.toLowerCase()==='f')flip();if(e.key.toLowerCase()==='r')reset();targetY=THREE.MathUtils.clamp(targetY,(flipped?Math.PI:0)-.65,(flipped?Math.PI:0)+.65);targetX=THREE.MathUtils.clamp(targetX,-.43,.43);});
